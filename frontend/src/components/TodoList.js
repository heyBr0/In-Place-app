...
export default function TodoList() {
  ...
  const createTask = (e) => {
    ...
    fetch("/tasks", {
      ...
      body: JSON.stringify({
        ...
        date: e.target.date.value === "" ? Date.now() : e.target.date.value,
        kanban: e.target.kanban.checked,
      }),
      headers: {
  ...
  return (
    <div className="container">
      {/*     <h2>To Do List</h2> */}
      <button onClick={addTaskButton} className="add-button">
        {plusIcon} Add Task
      </button>
      <div>
        {form && (
          <form onSubmit={createTask} className="create-task-form">
            ...
            <div className="checkbox">
              ...
              <label htmlFor="checkbox">Add to Kanban Board</label>
            </div>
          ...
          <div className="tasks-container">
            ...
            <div className="tasks-container-children">
              ...
              <ul>
                {doneTask.map((task) => {
                  const date = [
                    task.date.split("").splice(0, 4),
                    "/",
                    task.date.split("").splice(5, 2),
                    "/",
                    task.date.split("").splice(8, 2),
                  ];

                  return (
                    <div key={task._id} className="task-list-container done-list-container">
                      <li className="task-list" key={task._id}>
                        {task.task}
                      </li>
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            fetch(`tasks/completed/${task._id}`, {
                              ...
                              .then((result) => {
                                setUser((prevTask) => {
                                  const updated = prevTask.tasks.filter(
